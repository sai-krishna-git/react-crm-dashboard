...
router.get("/google/callback",
    ...
    async (req, res) => {
        try {
            ...
            const token = generateToken(user._id, user.role);
            let redirectURL = `http://localhost:3000/login?token=${token}&name=${encodeURIComponent(user.name)}&email=${encodeURIComponent(user.email)}`;
...
router.get("/github/callback",
    passport.authenticate("github", { failureRedirect: "http://localhost:3000/login?error=github_auth_failed" }),
    async (req, res) => {
        try {
            const user = req.user;
            if (!user) {
