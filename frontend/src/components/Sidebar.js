...
import { FaUsers, FaShoppingCart, FaBox, FaInfoCircle, FaChartLine, FaEnvelope } from "react-icons/fa";
...
const Sidebar = () => {
  ...
  const menuItems = [
    ...
    { icon: FaEnvelope, label: "Email Verification", path: "/email-verification" },
  ];
    ...
    <>
      ...
      <div className="p-4 md:hidden bg-blue-900">
        <button onClick={() => setIsOpen(!isOpen)}>
          {isOpen ? <MdClose size={24} className="text-white" /> : <MdMenu size={24} className="text-white" />}
        </button>
      </div>
